...
simu.simMechanicsFile = 'MECC_Buoy.slx';      % Specify Simulink Model File
...
% % Irregular Waves using JS Spectrum with Equal Energy and Seeded Phase
 %waves = waveClass('irregular');           % Initialize Wave Class and Specify Type
 %waves.height = 2;                       % Significant Wave Height [m]look at documentation, wave class 
 %waves.period = 8;                         % Peak Period [s]
...
%% Body Data
% Float
body(1) = bodyClass('hydroData/buoyhull.h5');      
...
body(1).mass = 122.7;
    ...
    % Weight.
body(1).inertia = [25.96 25.96 36.96];  % Moment of Inertia [kg*m^2] (Just the buoy shell)     
% Spar/Plate
...
mooring(1).moorDynNodes(1:2) = 16;              % Specify number of nodes per line
